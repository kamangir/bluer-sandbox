...
class Advertisement(ServiceInterface):
    ...
    @method()
    def Release(self):
        print("[Beacon] BlueZ requested Release() — advertisement unregistered.")
...
async def main():
    ...
    await bus.connect()
    print(f"[Beacon] Connected to system bus as {bus.unique_name}")

    # Register with BlueZ
    try:
        await register_advertisement(bus)
    except Exception as e:
        print(f"[Beacon] Failed to start advertising: {e}")
        return

    print("[Beacon] Advertising started as 'TEST-PI' (manuf 0xFFFF: <x,y,sigma>)")
    ...
    try:
        ...
        try:
            ...
            print("[Beacon] Unregistered advertisement.")
        except Exception as e:
            print(f"[Beacon] Unregister failed (ignored): {e}")
